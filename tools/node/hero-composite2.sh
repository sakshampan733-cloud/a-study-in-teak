...
$FF -loglevel error -y -t 7.6 -i bal-a.mp4 -vf "crop=800:450:120:250,scale=1600:900:flags=bicubic,gblur=sigma=$NB,setpts=$T/7.6*PTS,minterpolate=fps=24:mi_mode=blend,$CRUSH,format=gbrp,geq=r='r(X,Y)*(1-0.62*clip((b(X,Y)-r(X,Y))/55,0,1))':g='g(X,Y)*(1-0.62*clip((b(X,Y)-r(X,Y))/55,0,1))':b='b(X,Y)*(1-0.62*clip((b(X,Y)-r(X,Y))/55,0,1))',format=yuv420p" -t $T -an -c:v libx264 -crf 14 -pix_fmt yuv420p Bnear.mp4
# far: take B at 1.5x, mirrored so its columns travel the other way, larger and softer — the warm highlights
$FF -loglevel error -y -t 7.6 -i bal-b.mp4 -vf "hflip,crop=853:480:200:60,scale=1600:900:flags=bicubic,gblur=sigma=$FB,setpts=$T/7.6*PTS,minterpolate=fps=24:mi_mode=blend,$CRUSH,eq=brightness=-0.02" -t $T -an -c:v libx264 -crf 14 -pix_fmt yuv420p Bfar.mp4
$FF -loglevel error -y -i Bnear.mp4 -i Bfar.mp4 -loop 1 -i pool.png -filter_complex "\
[0]format=gbrp[n];[1]format=gbrp[f];[n][f]blend=all_mode=screen:all_opacity=${FO}[m];\
[m]format=yuv420p,eq=saturation=0.78,colorbalance=rm=0.02:bm=0.03:rh=0.03:gh=0.02[mm];[mm][2]overlay=0:0:shortest=1[v]" -map "[v]" -t $T -an -c:v libx264 -crf 14 -pix_fmt yuv420p Bcomp9.mp4

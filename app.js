...
function tabPage(t) {
  ...
  const items = t.items.map((i, k) => `
    <section class="item" id="${i.id}">
      ...
      ${i.video ? `<div class="wrap"><figure class="vid reveal"><video src="${esc(i.video.src)}" poster="${esc(i.video.poster)}" controls playsinline muted loop preload="metadata"></video><figcaption>${esc(i.video.caption)}</figcaption></figure></div>` : ""}
